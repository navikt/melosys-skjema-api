...
class ErFodselsEllerDNummerValidator(
    ...
    fun validate(fodselsnummer: String?, field: String = ""): List<Violation> {
        if (fodselsnummer == null) return emptyList()

        if (!validerFnrOgDnrFormat(fodselsnummer, erSyntetisk = validerSyntetiskFnr)) {
            return listOf(Violation(
                field = field,
                translationKey = "${ErrorMessageTranslation::fellesTranslation.name}.${FellesTranslation::ugyldigFodselsellerDNummer.name}"
            ))
        }
        return emptyList()
    }
    ...
    companion object {
         ...
         * @return true hvis ID-nummeret er gyldig, ellers false
         */
        fun validerFnrOgDnrFormat(gittNummer: String, erSyntetisk: Boolean = false): Boolean {
