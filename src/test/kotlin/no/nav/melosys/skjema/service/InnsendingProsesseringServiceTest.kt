...
class InnsendingProsesseringServiceTest : FunSpec({
    ...
    context("oppdaterStatus") {
        ...
        test("skal bevare journalpostId ved statusoppdatering") {
            val skjemaMetadata = createTestMetadata().copy(
...
private fun createTestMetadata() = UtsendtArbeidstakerMetadata(
...
private fun createTestSkjema(
    ...
    return Skjema(
        ...
        fnr = fnr,
        orgnr = orgnr,
        metadata = objectMapper.valueToTree(createTestMetadata()),
        opprettetAv = fnr,
        endretAv = fnr
    )
}
